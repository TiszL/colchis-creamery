...
export async function generateMetadata({ params, searchParams }: ShopPageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: { 'en': `${SITE_URL}/shop`, 'ka': `${SITE_URL}/ka/shop` },
    },
    openGraph: {
      type: 'website',
...
export default async function ShopPage({ params }: ShopPageProps) {
  ...
  const [dbProducts, creameryConfigs] = await Promise.all([
  ...
  const jsonLd = {
    ...
    mainEntity: {
      ...
      itemListElement: products.map((p, idx) => ({
        '@type': 'ListItem',
        position: idx + 1,
        url: `${SITE_URL}${prefix}/shop/${p.slug}`,
        name: p.name,
        image: p.imageUrl,
      })),
    },
            ...
            <div className="ch-cm-sub-grid" style={{ maxWidth: 1280, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "center" }}>
              <div>
                ...
                <div className="ch-cm-sub-ctas" style={{ marginTop: 32, display: "flex", gap: 14 }}>
                  <Link href={`${prefix}/shop`} style={{ background: "#8B4A28", color: "#1F3026", padding: "16px 30px", fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.28em", textTransform: "uppercase", textDecoration: "none" }}>{sub.cta_primary}</Link>
                  <Link href={`${prefix}/shop`} style={{ background: "transparent", color: "#F5F0E6", border: "1px solid #F5F0E655", padding: "16px 30px", fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: "0.28em", textTransform: "uppercase", textDecoration: "none" }}>{sub.cta_secondary}</Link>
                </div>
              </div>
