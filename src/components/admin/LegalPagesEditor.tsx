...
const DEFAULT_TERMS: LegalSection[] = [
    ...
    { heading: '4. Intellectual Property', body: 'The Site and its original content, features, functionalities, and branding are owned by Colchis Food LLC and are protected by international copyright, trademark, and other intellectual property laws.' },
                    ...
                    <div key={idx} className="bg-[#0C0C0C] border border-[#ffffff0A] p-4 space-y-3">
                        <div className="flex items-center gap-2">
                            ...
                            <input value={item.heading}
                                ...
                                className="flex-1 bg-transparent border border-[#B96A3D22] text-white py-2 px-3 focus:outline-none focus:border-[#B96A3D] text-sm font-medium" />
                        ...
                        <textarea value={item.body}
                            ...
                            className="w-full bg-transparent border border-[#B96A3D22] text-white py-2 px-3 focus:outline-none focus:border-[#B96A3D] text-sm resize-none" />
                    </div>
                ))}
                <button onClick={() => setItems([...items, { heading: '', body: '' }])}
                    className="flex items-center gap-2 text-[#B96A3D] text-xs font-bold hover:underline mt-2">
            ...
            <div className="flex gap-2 overflow-x-auto">
                {PAGES.map(p => (
                    <button key={p.id} onClick={() => setActiveTab(p.id)}
                        className={`flex items-center gap-2 px-4 py-2.5 text-sm font-bold transition-all border whitespace-nowrap ${activeTab === p.id
                            ? 'bg-[#ffffff08] text-white border-[#B96A3D22]'
                            : 'bg-transparent text-gray-500 border-transparent hover:text-white hover:bg-[#ffffff08]'}`}>
                            ...
                            <div key={idx} className="bg-[#0C0C0C] border border-[#ffffff0A] p-4 space-y-3">
                                <div className="flex items-center gap-2">
                                    ...
                                    <input value={item.question}
                                        ...
                                        className="flex-1 bg-transparent border border-[#B96A3D]/30 text-white py-2 px-3 focus:outline-none focus:border-[#B96A3D] text-sm font-medium" />
