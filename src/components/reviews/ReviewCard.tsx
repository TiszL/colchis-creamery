            ...
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#CBA153]/10 flex items-center justify-center shrink-0">
                        <span className="text-[#CBA153] font-bold text-sm">{displayName[0]?.toUpperCase()}</span>
                    </div>
                    <div>
            ...
            {(review.replies.length > 0 || isLoggedIn) && (
                <div className="border-t border-gray-100 pt-3 mt-3">
                    <button
                        ...
                        className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-[#CBA153] transition-colors"
                    >
                            ...
                            {review.replies.map((reply) => (
                                <div key={reply.id} className={`pl-4 border-l-2 ${reply.isAdminReply ? 'border-[#CBA153]' : 'border-gray-200'}`}>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        {reply.isAdminReply && (
                                            <span className="text-[9px] uppercase tracking-wider font-bold text-[#CBA153] bg-[#CBA153]/10 px-1.5 py-0.5 rounded">Official</span>
                                        )}
                            ...
                            {isLoggedIn && (
                                <div className="flex gap-2 mt-2">
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2 bg-[#CBA153] text-white text-xs font-bold rounded-lg hover:bg-[#b88e44] transition-colors disabled:opacity-50"
                                    >
                                        {replyLoading ? '...' : 'Reply'}
                                    </button>
                                </div>
