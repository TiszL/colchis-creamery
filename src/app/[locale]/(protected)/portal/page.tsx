...
export default async function StaffPortalDashboard({ params }: { params: any }) {
    ...
    const stats = [
        { label: 'Active Products', value: totalProducts.toString(), icon: Package, color: 'text-[#B96A3D]', roles: ['MASTER_ADMIN', 'PRODUCT_MANAGER'] },
    ...
    const quickActions = [
        {
            ...
            icon: Package, color: 'text-[#B96A3D]', bg: 'bg-[#B96A3D]/10',
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {visibleActions.map((action) => (
                    <Link
                        key={action.href}
                        href={action.href}
                        className="bg-[#161616] p-6 border border-[#ffffff0A] hover:border-[#B96A3D]/20 transition-all group"
                    >
                        <div className="flex items-start justify-between mb-4">
                            <div className={`w-10 h-10 ${action.bg} flex items-center justify-center`}>
                                <action.icon className={`w-5 h-5 ${action.color}`} />
                            </div>
                            <ArrowRight className="w-4 h-4 text-gray-600 group-hover:text-[#B96A3D] transition-colors" />
                        </div>
