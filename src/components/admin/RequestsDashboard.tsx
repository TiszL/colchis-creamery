...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: any }> = {
    ...
    CONTACTED: { label: 'Contacted', color: 'text-[#B96A3D]', bg: 'bg-[#B96A3D]/10 border-[#B96A3D]/20', icon: Mail },
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                {(['ALL', 'NEW', 'CONTACTED', 'CONVERTED', 'REJECTED'] as const).map((key) => {
                    ...
                    const config = key === 'ALL'
                        ? { label: 'All', color: 'text-white', bg: 'bg-[#ffffff08] border-[#B96A3D22]' }
                        : STATUS_CONFIG[key];
                    return (
                        <button
                            key={key}
                            ...
                            className={`p-3 border text-center transition-all ${
                                isActive
                                    ...
                                    : 'bg-[#161616] border-[#ffffff0A] hover:border-[#B96A3D22]'
                            }`}
                        >
                            ...
                            >
                                <div className="p-3 sm:p-4">
                                    <div className="flex items-start gap-3">
                                        <div className="w-11 h-11 sm:w-12 sm:h-12 bg-[#0C0C0C] border border-[#B96A3D22] flex items-center justify-center flex-shrink-0">
                                            <Building2 className="w-5 h-5 text-[#B96A3D]/60" />
                                        </div>
            ...
            {selectedLead && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-[#161616] border border-[#B96A3D22] w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        {/* Header */}
                        ...
                        <div className="p-6 space-y-5">
                            <div className="grid grid-cols-1 gap-4">
                                {selectedLead.contactName && (
                                    <div className="flex items-start gap-3">
                                        <Building2 className="w-4 h-4 text-[#B96A3D] mt-0.5 flex-shrink-0" />
                                        <div>
                                ...
                                <div className="flex items-start gap-3">
                                    <Mail className="w-4 h-4 text-[#B96A3D] mt-0.5 flex-shrink-0" />
                                    <div>
                                        <span className="text-[10px] text-gray-600 uppercase tracking-wider font-bold block">Email</span>
                                        <a href={`mailto:${selectedLead.email}`} className="text-[#B96A3D] text-sm hover:underline">{selectedLead.email}</a>
                                    </div>
                                </div>
                                {selectedLead.phone && (
                                    <div className="flex items-start gap-3">
                                        <Phone className="w-4 h-4 text-[#B96A3D] mt-0.5 flex-shrink-0" />
                                        <div>
                                ...
                                {selectedLead.address && (
                                    <div className="flex items-start gap-3">
                                        <MapPin className="w-4 h-4 text-[#B96A3D] mt-0.5 flex-shrink-0" />
                                        <div>
                                ...
                                {selectedLead.expectedVolume && (
                                    <div className="flex items-start gap-3">
                                        <Package className="w-4 h-4 text-[#B96A3D] mt-0.5 flex-shrink-0" />
                                        <div>
                                ...
                                {selectedLead.message && (
                                    <div className="flex items-start gap-3">
                                        <MessageSquare className="w-4 h-4 text-[#B96A3D] mt-0.5 flex-shrink-0" />
                                        <div>
                        ...
                        {generatedCode && (
                            <div className="mx-6 mb-4 bg-emerald-500/10 border border-emerald-500/20 p-4">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button
                                        onClick={copyCode}
                                        className="text-emerald-400 hover:text-white transition-colors p-1.5 hover:bg-[#ffffff08]"
                                    >
                        ...
                        {/* Action message */}
                        {actionMessage && (
                            <div className="mx-6 mb-4 text-sm text-gray-400 bg-[#ffffff08] px-4 py-2.5">
                                {actionMessage}
                            </div>
                        )}
                        ...
                        <div className="p-6 border-t border-[#ffffff0A]">
                            ...
                            {selectedLead.status === 'CONVERTED' ? (
                                <div className="flex items-center gap-2 text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 px-4 py-3">
                                ...
                                <div className="space-y-2">
                                    {selectedLead.status === 'NEW' && (
                                        <button
                                            ...
                                            disabled={!!updatingId}
                                            className="w-full flex items-center justify-center gap-2 py-2.5 bg-[#B96A3D]/10 text-[#B96A3D] border border-[#B96A3D]/20 text-xs font-bold uppercase tracking-wider hover:bg-[#B96A3D]/20 transition-colors disabled:opacity-50"
                                        >
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <button
                                            ...
                                            disabled={!!updatingId}
                                            className="flex items-center justify-center gap-2 py-3 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-bold uppercase tracking-wider hover:bg-emerald-500/20 transition-colors disabled:opacity-50"
                                        >
                                        ...
                                        <button
                                            ...
                                            disabled={!!updatingId}
                                            className="flex items-center justify-center gap-2 py-3 bg-red-500/10 text-red-400 border border-red-500/20 text-xs font-bold uppercase tracking-wider hover:bg-red-500/20 transition-colors disabled:opacity-50"
                                        >
                                            {updatingId === selectedLead.id ? (
                                                <Loader2 className="w-3.5 h-3.5 animate-spin" />
                                            ) : (
