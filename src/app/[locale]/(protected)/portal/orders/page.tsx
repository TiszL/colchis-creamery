                    ...
                    <table className="w-full text-left text-sm text-gray-400">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {orders.map((order: any) => (
                                <tr key={order.id} className="hover:bg-[#ffffff08] transition">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex px-2 py-1 text-[10px] font-bold uppercase rounded ${order.orderType === 'B2B' ? 'bg-[#B96A3D]/10 text-[#B96A3D]' : 'bg-white/10 text-gray-300'}`}>
                                            {order.orderType}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                    ...
                                    {!isSalesOnly && (
                                        <td className="px-6 py-4 border-l border-[#ffffff0A]">
                                            <form action={updateOrderStatus} className="flex flex-col gap-2">
                                                <input type="hidden" name="orderId" value={order.id} />
                                                <select
                                                    name="orderStatus"
                                                    defaultValue={order.orderStatus}
                                                    className="w-full text-xs bg-[#0C0C0C] border border-[#B96A3D22] text-white rounded py-1 px-2 focus:border-[#B96A3D]"
                                                >
                                                ...
                                                <input
                                                    ...
                                                    className="w-full text-xs bg-[#0C0C0C] border border-[#B96A3D22] text-white placeholder-gray-600 rounded py-1 px-2 focus:border-[#B96A3D]"
                                                />
                                                <button type="submit" className="w-full text-xs bg-[#B96A3D] text-black py-1.5 rounded font-bold hover:bg-white transition">
                                                    Update
                                                </button>
                                            </form>
                                        </td>
