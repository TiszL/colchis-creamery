...
export default async function PartnerSchedulesPage({
    ...
    return (
        <div className="max-w-4xl mx-auto px-6 py-10 space-y-8">
            <header>
                <h1 className="text-3xl font-serif text-white mb-2 flex items-center gap-2">
            ...
            {!partner && (
                <div className="bg-amber-950/30 border border-amber-900/40 px-4 py-3 text-amber-300 text-sm">
            ...
            <section>
                ...
                {schedules.length === 0 && (
                    <div className="bg-gray-900 border border-gray-800 p-6 text-center text-gray-500 text-sm">
                ...
                <div className="space-y-2">
                    {schedules.map(s => {
                        ...
                        return (
                            <div key={s.id} className={`bg-gray-900 border border-gray-800 px-4 py-3 flex items-center justify-between gap-4 ${!s.active ? "opacity-60" : ""}`}>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-3 mb-1">
                                        <p className="text-sm text-white">{s.name}</p>
                                        <span className="text-[10px] font-mono text-[#CBA153]">{s.paymentMethod.replace(/_/g, " ")}</span>
                                        {!s.active && <span className="text-[10px] font-mono text-amber-400 uppercase tracking-wider">Paused</span>}
                                    </div>
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                    <form action={toggleScheduleActiveAction}>
                                        ...
                                        <button type="submit" className="p-2 text-gray-400 hover:text-white" title={s.active ? "Pause" : "Resume"}>
                                            {s.active ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5" />}
                                        </button>
                                    </form>
                                    <form action={deleteScheduleAction}>
                                        <input type="hidden" name="id" value={s.id} />
                                        <button type="submit" className="p-2 text-gray-400 hover:text-red-400" title="Delete">
            ...
            {/* Create form */}
            {partner && (
                <section className="bg-gray-900 border border-gray-800 p-5">
                    <h2 className="text-lg font-semibold text-white mb-3">Add a schedule</h2>
                    <form action={async fd => { "use server"; await createScheduleAction(fd); }} className="space-y-3">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                ...
                                <input name="name" required placeholder="Weekly sulguni order" className="w-full bg-gray-950 border border-gray-800 text-white py-2 px-3 text-sm focus:outline-none focus:border-[#CBA153]" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Interval (days) *</label>
                                <input name="intervalDays" type="number" min={1} defaultValue={7} required className="w-full bg-gray-950 border border-gray-800 text-white py-2 px-3 text-sm focus:outline-none focus:border-[#CBA153]" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-wider">Payment method *</label>
                                <select name="paymentMethod" defaultValue="RESOLVE_NET_30" required className="w-full bg-gray-950 border border-gray-800 text-white py-2 px-3 text-sm focus:outline-none focus:border-[#CBA153]">
                            ...
                            <div>
                                ...
                                <select name="fulfillmentLocationId" defaultValue="" className="w-full bg-gray-950 border border-gray-800 text-white py-2 px-3 text-sm focus:outline-none focus:border-[#CBA153]">
                            ...
                            <div>
                                ...
                                <input name="firstFireAt" type="date" className="w-full bg-gray-950 border border-gray-800 text-white py-2 px-3 text-sm focus:outline-none focus:border-[#CBA153]" />
                            </div>
                        </div>
                        <div>
                            ...
                            <textarea
                                ...
                                className="w-full bg-gray-950 border border-gray-800 text-white py-2 px-3 text-sm font-mono focus:outline-none focus:border-[#CBA153]"
                            />
                        ...
                        <button type="submit" className="bg-[#CBA153] hover:bg-[#b08d47] text-white px-4 py-2 text-sm font-medium transition">
                            Create schedule
                        </button>
                    </form>
                </section>
